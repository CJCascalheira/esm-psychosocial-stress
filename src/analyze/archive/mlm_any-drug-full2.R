...
full_data1 <- full_data %>%
  ...
  mutate(
    ...
    grand_EDS = rescale(grand_EDS, to = c(0, 1)),
    grand_ACES = rescale(grand_ACES, to = c(0, 1)),
    grand_IPV = rescale(grand_IPV, to = c(0, 1)),
...
CI <- confint(l2_eds_race, parm = "beta_", method = "Wald")
logodds <- cbind(OR = fixef(l2_eds_race), CI)
ORwCI <- exp(logodds)
print(ORwCI, digits = 3)
...
# Convert to odds ratios
CI <- confint(l1_gen_race, parm = "beta_", method = "Wald")
logodds <- cbind(OR = fixef(l1_gen_race), CI)
ORwCI <- exp(logodds)
print(ORwCI, digits = 3)
...
# Convert to odds ratios
CI <- confint(l1_gen_income, parm = "beta_", method = "Wald")
logodds <- cbind(OR = fixef(l1_gen_income), CI)
ORwCI <- exp(logodds)
print(ORwCI, digits = 3)
...
l1_minority_race <- glmer(
  # control for previous day's drug use
  DD_any_drug_use ~ 1 + day + lag_DD_drug_use + 
    DD_minority_stress_total*is_poc +
    # Random intercept and stressor differs across participants
    (1 + DD_minority_stress_total|login_id),
  family = "binomial",
  data = full_data1,
...
# Convert to odds ratios
CI <- confint(l1_minority_race, parm = "beta_", method = "Wald")
logodds <- cbind(OR = fixef(l1_minority_race), CI)
ORwCI <- exp(logodds)
print(ORwCI, digits = 3)
...
l1_minority_gender <- glmer(
  # control for previous day's drug use
  DD_any_drug_use ~ 1 + day + lag_DD_drug_use + 
    DD_minority_stress_total*is_transgender +
    # Random intercept and stressor differs across participants
    (1 + DD_minority_stress_total|login_id),
  family = "binomial",
  data = full_data1,
