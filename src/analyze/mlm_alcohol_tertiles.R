...
full_data1 <- left_join(full_data, criterion_a_event) %>%
  ...
  mutate(
    lag_DD_alcohol_use = Lag(DD_alcohol_use, -1),
    lag_DD_alcohol_use = rescale(lag_DD_alcohol_use, to = c(0, 1))
  ) %>%
...
# ICC
null_model_aov <- aov(DD_alcohol_use ~ as.factor(login_id), full_data1)
ICC1(null_model_aov)
...
# Add lag of DV to control for alcohol use on previous day
lag_dv_model1 <- glmer(
...
# Should we include a random effect for yesterday's daily alcohol use?
anova(lag_dv_model1, lag_dv_model2)

# No, do not let the lag of daily alcohol use vary between person because no
...
# No! Therefore, retain model without RE for general perceived stress
...
anova(l1_stressors_gen, l1_gen_comp)
