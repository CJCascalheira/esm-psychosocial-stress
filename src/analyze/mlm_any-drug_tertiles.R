...
full_data1 <- left_join(full_data, criterion_a_event) %>%
  ...
  mutate(
    lag_DD_any_drug_use = Lag(DD_any_drug_use, -1),
    lag_DD_any_drug_use = rescale(lag_DD_any_drug_use, to = c(0, 1))
  ) %>%
...
lag_dv_model1 <- glmer(
  DD_any_drug_use ~ 1 + day + lag_DD_any_drug_use +
    (1|login_id),
  family = "binomial", 
  data = full_data1,
  nAGQ = 1, # Laplace approximation
...
lag_dv_model2 <- glmer(
  DD_any_drug_use ~ 1 + day + lag_DD_any_drug_use +
    (1 + lag_DD_any_drug_use|login_id),
  family = "binomial", 
  data = full_data1,
  nAGQ = 1, # Laplace approximation
  glmerControl(optimizer="bobyqa")
...
# No, do not let the lag of daily drug use vary between person because no
...
CI <- confint(l2_IPV_gender, parm = "beta_", method = "Wald", level = .95)
logodds <- cbind(OR = fixef(l2_IPV_gender), CI)
ORwCI <- exp(logodds)
print(ORwCI, digits = 5)
...
l1_gen_gender <- glmer(
  ...
  DD_any_drug_use ~ 1 + day + lag_DD_any_drug_use + 
    ...
    is_poc + above_25k + grand_age + 
    # Add stressors
    DD_gen_stress_total*is_transgender + mean_general_stress +
    # Random effects
    (1 + lag_DD_any_drug_use + DD_gen_stress_total|login_id),
  family = "binomial",
  data = full_data1,
  nAGQ = 1, # Use Laplace approximation 
  control = glmerControl(optimizer="bobyqa", optCtrl = list(maxfun = 2e5))
...
l1_minority_gender <- glmer(
  ...
  DD_any_drug_use ~ 1 + day + lag_DD_any_drug_use + 
    ...
    is_poc + above_25k + grand_age + 
    # Add stressor
    DD_minority_stress_total*is_transgender + mean_minority_stress +
    # Random effects
    (1|login_id),
  family = "binomial",
  data = full_data1,
